<template>
  <div class="canvasIndex">
    ...
    <div class="box">
      <div class="btn_group">
        <button class="btn" @click="add('arrow')">箭头</button>
        <button class="btn" @click="add('collect')">收音符</button>
      </div>
      <div class="text">
        <input type="text" v-model="someText" />
        <button class="addText" @click="addText">添加文字</button>
      </div>
      <button class="save" @click="save">保存</button>
      <button class="read" @click="read">读取</button>
      <input type="text" @keyup.delete="remove" ref="removeBox" />
    </div>
...
*/
  ...
  public save() {
    ...
    // console.log(typeof JSON.stringify(canvas.toJSON()));
  }
  public read() {
    if (this.jsonData) {
      const canvas = new fabric.Canvas('b');
      // const canvas = this._canvas;
  ...
  public remove() {
    ...
    // const findItem = itemArr.find(item => {
    //   return;
    // });
  }
}
