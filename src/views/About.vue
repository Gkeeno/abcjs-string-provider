...
`
    ...
    const abcstringBody = this.tunebookString.split(newline).splice(8);
  ...
  public breaktie() {
    ...
    const forward = this.tunebookString.substring(0, this.selectCharStart);
    const selected = this.tunebookString.slice(
  ...
  public adjustNotePitch(keysignal: KeyName) {
    ...
    const forward = this.tunebookString.substring(0, this.selectCharStart);
    const backward = this.tunebookString.substring(this.selectCharEnd);
    const iInSequence = this.selectCharPitch + 7; // 选中索引和构造的音符序列 需要索引对齐
    const key = SequenceNoteKey[iInSequence];
    if (!key) { return; }
